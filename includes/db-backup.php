...
function rwt_export_db() {
	...
	<h2><?php _e( 'Export complete!', 'wp-developers-toolbox' ) ?></h2>
	<h3><?php _e( 'File Path : ', 'wp-developers-toolbox' ) ?><?php echo $file_path; ?></h3>
	<h4><?php _e( '(Direct access is disabled with .htaccess for security - please use FTP / SSH to download)', 'wp-developers-toolbox' ) ?></h4>
<?php } // rwt_export_db ?>
...
<form name="rwt_export_db" method="post" action="">
	...
	<p><?php _e( 'Click the Export button below to export every table in the current database, to a .sql file.', 'wp-developers-toolbox' ) ?><br><?php _e( ' The file also includes “DROP TABLE IF EXISTS” for quick importing.', 'wp-developers-toolbox' ) ?></p>
	<p><input class="button-primary" type="submit" name="rwt_export_db_submit" value="<?php _e( 'Export', 'wp-developers-toolbox' ) ?>" /></p>
</form>
